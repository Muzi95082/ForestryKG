...
def main(_):
    ...
    train_y = np.load('./data_forestry/train_y.npy')
    train_word = np.load('./data_forestry/train_word.npy')
    train_pos1 = np.load('./data_forestry/train_pos1.npy')
    train_pos2 = np.load('./data_forestry/train_pos2.npy')
        ...
        with sess.as_default():
            ...
            def train_step(word_batch, pos1_batch, pos2_batch, y_batch, big_num):
                ...
                temp, step, loss, accuracy, summary, l2_loss, final_loss= sess.run(
                    ...
                    feed_dict)
                tp, tn, fp, fn=sess.run([m.tp_op, m.tn_op, m.fp_op, m.fn_op],feed_dict)
            ...
            for one_epoch in range(settings.num_epochs):
                ...
                for i in range(int(len(temp_order) / float(settings.big_num))):
                    ...
                    if current_step > 500 and current_step % 100 == 0:
                        print('saving model')
                        path = saver.save(sess, save_path + 'Forestry_ATT_GRU_model', global_step=current_step)
