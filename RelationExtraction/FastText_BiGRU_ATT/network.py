...
#实际值 tf.argmax(self.input_y[i],0)
#预测值 predictions[i]
            self.tn_op=[]
            self.tp_op=[]
            self.fp_op = []
            ...
            pdict=self.predictions[i]
            actuals=self.input_y[i]
            ones_like_actuals = tf.ones_like(actuals)
            zeros_like_actuals = tf.zeros_like(actuals)
            ones_like_predictions = tf.ones_like(pdict)
            zeros_like_predictions = tf.zeros_like(pdict)
            self.tp_op = tf.reduce_sum(
                tf.cast(
                    tf.logical_and(
                        tf.equal(actuals, ones_like_actuals),
                        tf.equal(pdict, ones_like_predictions)
                    ),
                    "float"
                )
            )
            self.tn_op = tf.reduce_sum(
                tf.cast(
                    tf.logical_and(
                        ...
                        tf.equal(pdict, zeros_like_predictions)
                    ),
                    "float"
                )
            )
