...
entity1 = deque()
entity2 = deque()
with codecs.open('TRAIN_FILE.TXT','r','utf-8')as tra,codecs.open('train.txt','a','utf-8') as writetra:
    linenum = 0
    for line in tra:
        linenum+=1
        if linenum%4==1:
            ...
            writetra.write(line[0:-2] + ',')
            print(line[0:-2])
            datas.append(line[1:-2].split())
        elif linenum%4==2:
            if line=="Other\r\n":
                labels.append(0)
                writetra.write('Other' + '\n')
            else:
                line = line.split('(')
                labels.append(relation2id[line[0]])
                writetra.write(line[0]+'\n')
