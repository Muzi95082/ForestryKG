...
def main(_):
    ...
    wordembedding = np.load('./data_nyt/vec.npy')
    print(wordembedding)

    print('reading training data')
    train_y = np.load('./data_nyt/train_y.npy')
    train_word = np.load('./data_nyt/train_word.npy')
    train_pos1 = np.load('./data_nyt/train_pos1.npy')
    train_pos2 = np.load('./data_nyt/train_pos2.npy')
            ...
            for one_epoch in range(settings.num_epochs):
                ...
                for i in range(int(len(temp_order) / float(settings.big_num))):
                    ...
                    if current_step > 500 and current_step % 100 == 0:
                        print('saving model')
                        path = saver.save(sess, save_path + 'NYT_ATT_GRU_model', global_step=current_step)
