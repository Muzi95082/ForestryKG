...
def init():
    ...
    vec = []
    word2id = {}
    f = open('./forestry_data/vector.txt', encoding='utf-8')
    content = f.readline()
    content = content.strip().split()
    dim = int(len(content)-1)
    while True:
        content = f.readline()
        if content == '':
            break
    ...
    relation2id = {}
    f = open('./forestry_data/relation2id.txt', 'r', encoding='utf-8')
    while True:
        content = f.readline()
        if content == '':
            break
    ...
    f = open('./forestry_data/train.txt', 'r', encoding='utf-8')
    ...
    f = open('./forestry_data/test.txt', 'r', encoding='utf-8')
    ...
    f = open('./data_forestry/train_q&a.txt', 'w', encoding='utf-8')
    temp = 0
    for i in train_sen:
    ...
    f = open('./data_forestry/test_q&a.txt', 'w', encoding='utf-8')
    temp = 0
    for i in test_sen:
    ...
    np.save('./data_forestry/vec.npy', vec)
    np.save('./data_forestry/train_x.npy', train_x)
    np.save('./data_forestry/train_y.npy', train_y)
    np.save('./data_forestry/testall_x.npy', test_x)
    np.save('./data_forestry/testall_y.npy', test_y)
...
def seperate():
    ...
    x_train = np.load('./data_forestry/train_x.npy')
    ...
    np.save('./data_forestry/train_word.npy', train_word)
    np.save('./data_forestry/train_pos1.npy', train_pos1)
    np.save('./data_forestry/train_pos2.npy', train_pos2)

    print('seperating test all data')
    x_test = np.load('./data_forestry/testall_x.npy')
    test_word = []
    test_pos1 = []
    test_pos2 = []
    ...
    np.save('./data_forestry/testall_word.npy', test_word)
    np.save('./data_forestry/testall_pos1.npy', test_pos1)
    np.save('./data_forestry/testall_pos2.npy', test_pos2)



# get answer metric for PR curve evaluation
def getans():
    test_y = np.load('./data_forestry/testall_y.npy')
    eval_y = []
    for i in test_y:
        eval_y.append(i[1:])
    allans = np.reshape(eval_y, (-1))
    np.save('./data_forestry/allans.npy', allans)


def get_metadata():
    fwrite = open('./data_forestry/metadata.tsv', 'w', encoding='utf-8')
    f = open('./forestry_data/vector.txt', encoding='utf-8')
    f.readline()
    while True:
